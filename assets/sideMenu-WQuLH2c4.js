(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const o of document.querySelectorAll('link[rel="modulepreload"]'))s(o);new MutationObserver(o=>{for(const n of o)if(n.type==="childList")for(const i of n.addedNodes)i.tagName==="LINK"&&i.rel==="modulepreload"&&s(i)}).observe(document,{childList:!0,subtree:!0});function t(o){const n={};return o.integrity&&(n.integrity=o.integrity),o.referrerPolicy&&(n.referrerPolicy=o.referrerPolicy),o.crossOrigin==="use-credentials"?n.credentials="include":o.crossOrigin==="anonymous"?n.credentials="omit":n.credentials="same-origin",n}function s(o){if(o.ep)return;o.ep=!0;const n=t(o);fetch(o.href,n)}})();class p{constructor(e){this.currentStep=0,this.screens=document.querySelectorAll(e),this.init()}init(){document.addEventListener("click",this.handleClick.bind(this))}handleClick(e){const t=e.target;t&&(t.classList.contains("next_popup_screen")&&this.next(),t.classList.contains("prev_popup_screen")&&this.prev())}next(){this.currentStep<this.screens.length-1&&(this.toggleScreen(this.currentStep,this.currentStep+1),this.currentStep++)}prev(){this.currentStep>0&&(this.toggleScreen(this.currentStep,this.currentStep-1),this.currentStep--)}toggleScreen(e,t){this.screens.item(e).classList.remove("popup__screen--active"),this.screens.item(t).classList.add("popup__screen--active")}}class h{constructor(e){if(typeof e=="string"){const t=document.querySelector(e);if(console.log(e),!t)throw new Error(`Dropdown: the element is not found "${e}"`);this.root=t}else this.root=e;console.log(this.root),this.trigger=this.root.querySelector(".dropdown__trigger"),this.value=this.root.querySelector(".dropdown__value"),this.options=this.root.querySelectorAll(".dropdown__menu a"),this.init()}init(){this.trigger.addEventListener("click",e=>{e.stopPropagation(),console.log("trigger clicked"),this.toggle()}),this.options.forEach(e=>{e.addEventListener("click",t=>{t.preventDefault(),this.select(e)})})}toggle(){document.querySelectorAll(".dropdown").forEach(e=>{e!==this.root&&e.classList.remove("open")}),this.root.classList.toggle("open")}select(e){this.value.textContent=e.textContent??"",this.options.forEach(t=>t.classList.remove("selected")),e.classList.add("selected"),this.root.classList.remove("open")}}class m{constructor(e,t,s){this.title=e,this.popupId=t,this.stateController=s,this.render(),this.init()}render(){const e=document.createElement("div");e.className="popup donation__popup",e.id=this.popupId,e.setAttribute("popover",""),e.innerHTML=`
    ...
    `,this.popup=e,document.body.appendChild(e)}init(){this.popup.addEventListener("close-popup",()=>{this.close()}),this.popup.addEventListener("toggle",e=>{const t=Array.isArray(this.content)?this.content:[this.content];e.newState==="open"&&(t.forEach(s=>{"onOpen"in s&&s?.onOpen?.()}),this.stateController?.reset()),e.newState==="closed"&&t.forEach(s=>{"onClose"in s&&s.onClose?.()})})}normalizeContent(e){return this.content=e,e instanceof HTMLElement?[e]:Array.isArray(e)?e.map(t=>t.getScreen()):[e.getScreen()]}setContent(e){const t=this.popup.querySelector(".popup__inner");t&&this.normalizeContent(e).forEach(s=>t.appendChild(s))}open(){this.popup.showPopover?.(),"onOpen"in this.content&&this.content?.onOpen?.()}close(){this.popup.hidePopover?.(),"onClose"in this.content&&this.content?.onClose?.()}}function l(r){const e=/^(?=.*[!@#$%^&*(),.?":{}|<>]).{6,}$/;return r?r.length<6?"Password must be at least 6 characters":e.test(r)?null:"Password must contain at least 1 special character":"Password is required"}function u(r){const e=/^[^\s@]+@[^\s@]+\.[^\s@]+$/;return r?e.test(r)?null:"Invalid email format":"Email is required"}function c(r){const e=/^[A-Za-z0-9]+$/,t=/^[a-zA-Z]/;return r?e.test(r)?t.test(r)?r.length<3?"Login must be at least 3 characters":null:"Login must starts with latin letter":"Login must contain only latin letters and numbers":"Login is required"}function d(r){const e=/^[A-Za-z0-9 ]+$/,t=/^[a-zA-Z]/;return r?e.test(r)?t.test(r)?r.length<3?"Name must be at least 3 characters":null:"Name must starts with latin letter":"Name must contain only latin letters and numbers":"Name is required"}function E(r){return r?/^\d+(\.\d+)?$/.test(r)?Number(r)<=0?"Amount must be greater than 0":null:"Amount must be a valid number":"Amount is required"}function q(r){const e=/^\d{16}$/;return r?e.test(r)?null:"Card number must contain exactly 16 digits":"Card number is required"}function x(r){const e=/^\d{3}$/;return r?e.test(r)?null:"CVV must be exactly 3 digits":"CVV is required"}function a(r,e,t){r.addEventListener("blur",()=>f(r,e,t)),r.addEventListener("focus",()=>g(r,e,t))}function f(r,e,t){const s=t(r.value);s?(r.classList.add("error-input"),e.classList.remove("hidden"),e.textContent=s):(e.classList.add("hidden"),e.textContent="")}function g(r,e,t){const s=r.value;t(s)&&(r.classList.remove("error-input"),e.classList.add("hidden"))}class v{constructor(){this.screen=this.render(),this.setUp(),this.init()}render(){const e=document.createElement("div");return e.classList.add("popup__screen"),e.classList.add("popup__screen--active"),e.classList.add("login-group__screen"),e.innerHTML=`
    ...
    `,e}init(){a(this.loginInput,this.loginError,c),a(this.nameInput,this.nameError,d),a(this.emailInput,this.emailError,u),a(this.passwordInput,this.passwordError,l),a(this.confirmInput,this.confirmError,this.validateConfirmPassword),this.form.addEventListener("input",()=>{this.onFormInput()}),this.form.addEventListener("submit",async e=>{e.preventDefault(),await this.submitForm()})}validateConfirmPassword(e){return e?this.passwordInput.value!==e?"Password doesn't match":null:"Please confirm password"}onFormInput(){const e=this.loginInput.value,t=this.nameInput.value,s=this.emailInput.value,o=this.passwordInput.value,n=this.confirmInput.value;this.formSpan.classList.add("hidden"),this.formSpan.textContent="",!c(e)&&!d(t)&&!l(o)&&!u(s)&&!this.validateConfirmPassword(n)?this.submitButton.removeAttribute("disabled"):this.submitButton.setAttribute("disabled","")}async submitForm(){const e=new FormData(this.form),t=Object.fromEntries(e.entries());this.submitButton.setAttribute("disabled","");try{const s=await fetch("https://vsqsnqnxkh.execute-api.eu-central-1.amazonaws.com/prod/auth/register",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(t)});if(s.ok){const o=await s.json(),{access_token:n,user:i}=o.data;localStorage.setItem("token",n),localStorage.setItem("user",JSON.stringify(i)),window.location.assign("index.html")}else{const o=await s.json();this.formSpan.textContent=o.error,this.formSpan.classList.remove("hidden"),this.submitButton.removeAttribute("disabled"),console.error("Server error:",s.status)}}catch(s){this.formSpan.textContent="Something went wrong. Try again",this.formSpan.classList.remove("hidden"),console.error("x3:",s)}}resetForm(){this.form.reset(),this.screen.querySelectorAll(".form-error").forEach(t=>{t.textContent="",t.classList.add("hidden")}),this.submitButton.setAttribute("disabled","")}getScreen(){return this.screen}onOpen(){this.resetForm()}}class w{constructor(e){this.root=e}render(){if(localStorage.getItem("token")&&localStorage.getItem("user")){const{name:e}=JSON.parse(localStorage.getItem("user"));this.root.textContent=e}else{const e=document.createElement("button");e.classList.add("login-button"),e.setAttribute("popovertarget","login-popup"),e.innerHTML=`
      ...
      `,this.root.appendChild(e)}}}document.querySelectorAll(".dropdown").forEach(r=>new h(r));document.addEventListener("click",()=>{document.querySelectorAll(".dropdown").forEach(r=>r.classList.remove("open"))});const L=document.querySelector(".user__container"),_=new w(L);_.render();const y=new m("Login/Registration","login-popup"),S=new v,C=new b;y.setContent([S,C]);new p(".login-group__screen");class I{constructor(e,t,s){this.container=e,this.items=t,this.activeIndex=s}render(){const e=this.items.map((t,s)=>`
        <li>
          <a class="${s===this.activeIndex?"active":""}" href="${t.href}">
            ${t.label}
          </a>
        </li>
    ...
    `}}export{m as B,h as D,p as P,I as S,d as a,a as b,u as c,q as d,x as e,E as v};
