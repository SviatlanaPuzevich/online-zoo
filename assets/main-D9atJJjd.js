import{v as a,b as r,a as c,c as l,d as p,e as v,S as C,B as b,P as f}from"./sideMenu-Di2GFJp8.js";import{L as u,A as h,a as m}from"./alert-rXC1ack9.js";class _{constructor(t){this.review=t}render(){const t=document.createElement("div");return t.className="reviews-item",t.innerHTML=`
    ...
    `,t}}class g{constructor(t,e){this.container=t,this.animal=e}render(){const t=document.createDocumentFragment();this.animal.forEach(e=>{const n=new _(e);t.appendChild(n.render())}),this.container.appendChild(t)}}class w{constructor(t){this.animal=t,this.BASE="/online-zoo/"}render(){const t=document.createElement("div");return t.className="animal-card",t.innerHTML=`
    ...
    `,t}setValuesFromState(){}setUp(){this.creditCardInput=this.screen.querySelector("#payment-credit-card"),this.creditCardError=this.screen.querySelector("#payment-credit-card-error"),this.cvvInput=this.screen.querySelector("#payment-cvv"),this.cvvError=this.screen.querySelector("#payment-cvv-error"),this.backButton=this.screen.querySelector("#payment-back"),this.completeButton=this.screen.querySelector("#payment-complete-button"),this.saveCardCheckbox=this.screen.querySelector("#save-card-checkbox")}init(){r(this.creditCardInput,this.creditCardError,p),r(this.cvvInput,this.cvvError,v),this.completeButton.addEventListener("click",()=>{}),this.backButton.addEventListener("click",()=>this.saveState()),this.syncCheckboxFromStorage(),this.saveCardCheckbox.addEventListener("change",()=>{this.saveCardCheckbox.checked||localStorage.removeItem("savedCard")})}saveState(){const t=Number(this.creditCardInput.value);t&&this.paymentController.update({cardNumber:t}),this.saveCardCheckbox.checked&&localStorage.setItem("savedCard",JSON.stringify({cardNumber:this.paymentController.getState().cardNumber,expiry:this.paymentController.getState().expDate}))}syncCheckboxFromStorage(){const t=localStorage.getItem("savedCard");this.saveCardCheckbox.checked=!!t}resetForm(){this.creditCardInput.value="",this.creditCardError.textContent="",this.cvvInput.value="",this.cvvError.textContent="",this.completeButton.disabled=!0}getScreen(){return this.screen}onOpen(){this.resetForm(),this.setValuesFromState()}}const q=[{label:"About",href:"index.html"},{label:"Map",href:"./src/pages/map/index.html"},{label:"Zoos",href:"./src/pages/zoos/index.html?id=1"},{label:"Contact Us",href:"./src/pages/contacts/index.html"},{label:"Design",href:"figma.com/file/lnK11foY8Aoa6oOlDXovVN/Online-ZOO-Project?node-id=0%3A1"}],A=document.getElementById("side-menu"),N=new C(A,q,0);N.render();const d=document.querySelector("#animal-slider");async function F(){const i=document.querySelector("#animal-cards"),t=new u().render();i.appendChild(t);try{const e=await h.getAnimals();t.remove(),new x(i,e).render(),new y(d)}catch{t.remove();const n=new m("Something went wrong. Please, refresh the page").render();d.prepend(n)}}async function M(){const i=document.querySelector("#reviews-slider"),t=document.querySelector("#reviews-wrapper"),e=new u().render();t.appendChild(e);try{const n=await h.getReviews();e.remove(),new g(t,n).render(),new S(i)}catch{e.remove();const s=new m("Something went wrong. Please, refresh the page").render();i.prepend(s)}}const o=new L,W=new E(o),H=new I(o),$=new B(o),P=new b("Make your donation","donation-popup",o);P.setContent([W,H,$]);new f(".donation__screen");const R=document.querySelectorAll(".donate-card__button");R.forEach(i=>{i.setAttribute("popovertarget","donation-popup")});F();M();
