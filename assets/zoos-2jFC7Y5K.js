import{B as d,S as p}from"./sideMenu-BbNfAaIE.js";import{L as l,A as o,a as c}from"./alert-rXC1ack9.js";class m{constructor(e,t){this.root=e,this.fact=t}render(){const e="map-popup";this.root.innerHTML=`
    ...
    `,new d("Animal Location",e).setContent(this.createPopupContent())}parseCoordinate(e){const t=e.match(/([\d.]+)°\s*([NSEW])/);if(!t)return 0;const s=parseFloat(t[1]),a=t[2];return a==="S"||a==="W"?-s:s}createPopupContent(){const e=this.parseCoordinate(this.fact.latitude),t=this.parseCoordinate(this.fact.longitude),s=`https://www.google.com/maps?q=${e},${t}&z=12&output=embed`,a=document.createElement("iframe");return a.src=s,a.className="map-frame",a.setAttribute("loading","lazy"),a}}class h{constructor(e,t,s){this.root=e,this.data=t,this.activeId=s,this.currentGroup=1,this.visibleCount=3;const a=e.querySelector("#collapseBtn"),r=e.querySelector("#drawer-list");if(this.downButton=e.querySelector("#drawer-down-button"),!a||!this.downButton)throw new Error("Drawer: elements not found");this.collapseButton=a,this.animalList=r,this.drawerBadge=e.querySelector("#drawer-badge"),this.drawerToggleIcon=e.querySelector("#drawerToggleIcon"),this.iconWrappers=e.querySelectorAll("#drawer .drawer__icon-wrap"),this.circleWrappers=e.querySelectorAll("#drawer .drawer__circle"),this.init()}renderList(){this.animalList.appendChild(this.renderActiveItem());for(let e=0;e<this.data.length;e++){const t=e<this.visibleCount;this.animalList.appendChild(this.renderItem(this.data[e],t))}}renderItem(e,t){const s=document.createElement("li");return s.classList.add("drawer__item"),t||s.classList.add("hidden"),s.innerHTML=`
                                ...
                                <div class="drawer__circle drawer__circle--collapsed">
                                    <div class="drawer__icon-wrap drawer__icon-wrap--collapsed">
                                        <svg class="drawer__icon">
                                            <use href="./assets/icons/zoos/sprite.svg#${e.id}-d-icon"></use>
                                        </svg>
                                    </div>
                                </div>
                                <p class="drawer__text">${e.text}</p>
                            </a>
    `,s}renderActiveItem(){const e=this.data.find(s=>s.petId===this.activeId),t=document.createElement("li");if(t.classList.add("drawer__item"),t.classList.add("drawer__item--active"),e){const s=this.data.findIndex(a=>a.id===this.activeId);this.data.splice(s,1),t.innerHTML=`
    <a class="drawer__link" href="index.html?id=${e.petId}">
                                <div class="drawer__circle drawer__circle--collapsed">
    ...
    `}return t}init(){this.collapseButton.addEventListener("click",e=>{e.preventDefault(),this.toggleLeftRight()}),this.downButton.addEventListener("click",e=>{this.showNext(),e.preventDefault()})}toggleLeftRight(){console.log("expand"),this.collapseButton.classList.toggle("drawer__collapse--expanded"),this.root.classList.toggle("drawer--expanded"),this.drawerBadge?.classList.toggle("drawer__badge--visible"),this.drawerToggleIcon?.classList.toggle("drawer__left-icon--expanded"),this.circleWrappers.forEach(e=>{e.classList.toggle("drawer__circle--collapsed")}),this.iconWrappers.forEach(e=>{e.classList.contains("drawer__icon-wrap--active")?e.classList.toggle("drawer__icon-wrap--active--collapsed"):e.classList.toggle("drawer__icon-wrap--collapsed")})}showNext(){const e=Array.from(this.animalList.children),t=this.currentGroup*this.visibleCount+1,s=t+this.visibleCount;e.forEach((a,r)=>{r!==0&&a.classList.add("hidden")});for(let a=t;a<s;a++)e[a]&&e[a]?.classList.remove("hidden");if(this.currentGroup++,t>=this.data.length){this.currentGroup=1;for(let a=1;a<=this.visibleCount;a++)e[a]?.classList.remove("hidden")}}}const _=[{label:"About",href:"../../../index.html"},{label:"Map",href:"../map/index.html"},{label:"Zoos",href:"index.html"},{label:"Contact Us",href:"../contacts/index.html"},{label:"Design",href:"figma.com/file/lnK11foY8Aoa6oOlDXovVN/Online-ZOO-Project?node-id=0%3A1"}],f=document.getElementById("side-menu"),w=new p(f,_,2);w.render();const u=new URLSearchParams(window.location.search),n=Number(u.get("id"));if(!n)throw new Error("id parameter is missing");(function(e){const t=[`../../../assets/images/zoos/${e}/mainCam.png`,`../../../assets/images/zoos/${e}/cam1.png`,`../../../assets/images/zoos/${e}/cam2.png`,`../../../assets/images/zoos/${e}/cam3.png`],s=Array.from(document.querySelectorAll(".video__link > img"));for(let a=0;a<s.length;a++)s[a].setAttribute("src",t[a])})(n);async function g(){const i=document.getElementById("drawer"),e=new l().render();i.appendChild(e);try{const t=await o.getCameras();e.remove(),new h(i,t,n).renderList()}catch{e.remove();const s=new c("Something went wrong. Please, refresh the page").render();i.prepend(s)}}async function v(){const i=document.getElementById("fact-container"),e=new l().render();i.appendChild(e);try{const t=await o.getAnimalFact(n);e.remove(),new m(i,t).render()}catch{e.remove();const s=new c("Something went wrong. Please, refresh the page").render();i.prepend(s)}}g();v();
