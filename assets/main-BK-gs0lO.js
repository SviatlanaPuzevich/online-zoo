import{v as a,b as r,a as h,c as m,d as p,e as v,S as g,B as _,P as y,D as l}from"./sideMenu-WQuLH2c4.js";import{A as d,L as f,a as b}from"./alert-9mdM6_HT.js";class x{constructor(t){this.review=t}render(){const t=document.createElement("div");return t.className="reviews-item",t.innerHTML=`
    ...
    `,t}}class S{constructor(t,e){this.container=t,this.animal=e}render(){const t=document.createDocumentFragment();this.animal.forEach(e=>{const n=new x(e);t.appendChild(n.render())}),this.container.appendChild(t)}}class L{constructor(t){this.animal=t,this.BASE="/online-zoo/"}render(){const t=document.createElement("div");return t.className="animal-card",t.innerHTML=`
    ...
    `,t}setValuesFromState(){}setUp(){this.creditCardInput=this.screen.querySelector("#payment-credit-card"),this.creditCardError=this.screen.querySelector("#payment-credit-card-error"),this.cvvInput=this.screen.querySelector("#payment-cvv"),this.cvvError=this.screen.querySelector("#payment-cvv-error"),this.backButton=this.screen.querySelector("#payment-back"),this.completeButton=this.screen.querySelector("#payment-complete-button"),this.saveCardCheckbox=this.screen.querySelector("#save-card-checkbox"),this.monthValue=this.screen.querySelector("#month-dropdown .dropdown__value"),this.yearValue=this.screen.querySelector("#year-dropdown .dropdown__value"),this.monthMenu=this.screen.querySelector("#month-dropdown .dropdown__menu"),this.yearMenu=this.screen.querySelector("#year-dropdown .dropdown__menu")}init(){r(this.creditCardInput,this.creditCardError,p),r(this.cvvInput,this.cvvError,v),this.completeButton.addEventListener("click",()=>{}),this.backButton.addEventListener("click",()=>this.saveState()),this.syncCheckboxFromStorage(),this.saveCardCheckbox.addEventListener("change",()=>{this.saveCardCheckbox.checked||localStorage.removeItem("savedCard")}),this.creditCardInput.addEventListener("input",()=>this.inputHandle()),this.cvvInput.addEventListener("input",()=>this.inputHandle()),this.monthMenu.addEventListener("click",t=>{const e=t.target;e.tagName==="A"&&(this.monthValue.textContent=e.textContent,this.inputHandle())}),this.yearMenu.addEventListener("click",t=>{const e=t.target;e.tagName==="A"&&(this.yearValue.textContent=e.textContent,this.inputHandle())}),this.completeButton.addEventListener("click",async()=>{await this.handleComplete()})}inputHandle(){const t=!p(this.creditCardInput.value)&&this.creditCardInput.value!=="",e=!v(this.cvvInput.value)&&this.cvvInput.value!=="",n=this.monthValue.textContent!=="Month",s=this.yearValue.textContent!=="Year";this.completeButton.disabled=!(t&&e&&n&&s)}saveState(){const t=Number(this.creditCardInput.value);t&&this.paymentController.update({cardNumber:t}),this.saveCardCheckbox.checked&&localStorage.setItem("savedCard",JSON.stringify({cardNumber:this.paymentController.getState().cardNumber,expiry:this.paymentController.getState().expDate}))}syncCheckboxFromStorage(){const t=localStorage.getItem("savedCard");this.saveCardCheckbox.checked=!!t}async handleComplete(){const{name:t,email:e,amount:n,petId:s,petName:o}=this.paymentController.getState();try{await d.sendDonation({name:t,email:e,amount:n,petId:s}),console.log("success"),M(n,o||"your favourite animal");const u=new CustomEvent("close-popup",{bubbles:!0});this.screen.dispatchEvent(u)}catch{console.log("Failed to save donation"),V()}}resetForm(){this.creditCardInput.value="",this.creditCardError.textContent="",this.cvvInput.value="",this.cvvError.textContent="",this.completeButton.disabled=!0,this.monthValue.textContent="Month",this.yearValue.textContent="Year"}getScreen(){return this.screen}onOpen(){this.resetForm(),this.setValuesFromState(),this.inputHandle()}}const H=[{label:"About",href:"index.html"},{label:"Map",href:"./src/pages/map/index.html"},{label:"Zoos",href:"./src/pages/zoos/index.html?id=1"},{label:"Contact Us",href:"./src/pages/contacts/index.html"},{label:"Design",href:"figma.com/file/lnK11foY8Aoa6oOlDXovVN/Online-ZOO-Project?node-id=0%3A1"}],$=document.getElementById("side-menu"),T=new g($,H,0);T.render();const C=document.querySelector("#animal-slider");async function W(){const i=document.querySelector("#animal-cards"),t=new f().render();i.appendChild(t);try{const e=await d.getAnimals();t.remove(),new k(i,e).render(),new E(C)}catch{t.remove();const n=new b("Something went wrong. Please, refresh the page").render();C.prepend(n)}}async function P(){const i=document.querySelector("#reviews-slider"),t=document.querySelector("#reviews-wrapper"),e=new f().render();t.appendChild(e);try{const n=await d.getReviews();e.remove(),new S(t,n).render(),new I(i)}catch{e.remove();const s=new b("Something went wrong. Please, refresh the page").render();i.prepend(s)}}const c=new B,R=new A(c),D=new N(c),U=new F(c),O=new _("Make your donation","donation-popup",c);O.setContent([R,D,U]);new y(".donation__screen");new l("#animal-dropdown");new l("#month-dropdown");new l("#year-dropdown");const z=document.querySelectorAll(".donate-card__button");z.forEach(i=>{i.setAttribute("popovertarget","donation-popup")});W();P();
