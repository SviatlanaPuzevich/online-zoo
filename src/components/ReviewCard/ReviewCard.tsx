...
const ReviewCard: React.FC<ReviewCardProps> = ({ review }) => {
  const { city, month, year, text, name } = review;
  ...
  return (
    <div className={styles.review}>
      ...
      <div className={classNames(styles.review__title, 'subheader')}>
        {city}, {month} {year}
      </div>
      <div className={styles.review__text}>{text}</div>
      <div className={styles.review__author}>{name}</div>
    </div>
  );
};
